...
def train_val_test():
	...
	#数据路径
    train_dir = 'D://ZY//DataEnhance0709train'
    val_dir = 'D://ZY//DataEnhance0709develop'
	# 构建Dataset实例
    train_datasets = torchvision.datasets.ImageFolder(train_dir, transform=train_transforms)
    val_datasets = torchvision.datasets.ImageFolder(val_dir, transform=val_transforms)
	#define data loader
    train_loader= torch.utils.data.DataLoader(
			train_datasets, batch_size=32, shuffle=True, num_workers=0
			)
    val_loader= torch.utils.data.DataLoader(
		val_datasets, batch_size=32, shuffle=False,
		num_workers=0
	)
